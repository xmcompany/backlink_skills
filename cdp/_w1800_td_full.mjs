...
await cdp.send('Page.navigate', { url: 'https://www.techdirectory.io/get-listed' });
await sleep(8000);
console.log('at:', await ev(`location.href.slice(0,80)`));

// 找三个 CREATE LISTING 中第一个(FREE)
const btn = await ev(`(function(){
  var btns=[...document.querySelectorAll('a,button,input')].filter(function(e){
    return /create listing/i.test((e.innerText||e.value||'')) && e.getBoundingClientRect().width>0;
  });
  if(!btns.length) return 'NF';
  var el=btns[0];
  el.scrollIntoView({block:'center'});
  var r=el.getBoundingClientRect();
  return JSON.stringify({x:r.x+r.width/2, y:r.y+r.height/2, tag:el.tagName, href:el.href||''});
})()`);
console.log('free btn:', btn);
let p;
try { p = JSON.parse(btn); } catch(e) { console.log('no free btn'); process.exit(1); }
...
await sleep(8000);
console.log('after click:', await ev(`location.href.slice(0,90)`));
